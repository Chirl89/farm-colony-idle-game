# Plan de Iteracion v4 - Aetheria: Granja & Colonia
### v4 -- Nuevas mecanicas M1-M18 anadidas + plan original A-V completo (2026-06-25)

> **Reglas:**
> - Cada paso indica archivos exactos a modificar. Solo toca esos.
> - Al completar un paso, avisa para marcarlo completado y moverlo a docs/backlog.md.
> - Backups = pulsar boton "Backup ZIP" en la UI del juego.
> - CSVs de test se generan al completar cada mecanica (ver test/README.md).
> - ui-render.js tiene ~170KB -- usar grep_search antes de view_file.

---

## YA IMPLEMENTADO (no tocar)

| Sistema | Archivos | Notas |
|---------|----------|-------|
| Estructura multi-fichero | index.html, src/js/, src/css/, src/data/ | FR-1 a FR-13 OK |
| Estado global (gamestate.js) | src/js/gamestate.js | save/load/reset OK |
| Reloj in-game + cooldown 30min | src/js/gameloop.js | Paso 1 OK |
| Ciclo dia/noche | src/js/gameloop.js | Configurable en timings.csv OK |
| Recoleccion manual (madera/piedra/frutos) | src/js/buildings.js, ui-events.js | OK |
| Asignacion colonos a recoleccion | src/js/ui-render.js, ui-events.js | OK |
| Etapas de cultivo (plow/sow/water/grow) | src/js/gameloop.js, ui-render.js | OK |
| Sub-recursos de comida | src/js/gamestate.js | OK |
| Sistema de semillas (buy/granero) | src/js/buildings.js, prices.csv | OK |
| Granero (produce semillas de cultivos) | src/js/buildings.js, gameloop.js | 3 tiers OK |
| Fogata / Caldero / Cocina | src/js/buildings.js, gameloop.js | 3 tiers OK |
| Modelo colonos con 8 atributos + XP | src/js/gamestate.js | OK |
| 3 candidatos para contratar | src/js/ui-render.js | OK |
| Asignacion colonos a edificios | src/js/ui-render.js, ui-events.js | OK |
| Mercado automatico + manual | src/js/gameloop.js, ui-render.js | OK |
| Ayuntamiento T1/T2/T3 (bloquea construcciones) | src/js/buildings.js, ui-render.js | OK |
| Backup ZIP + Importar CSV | src/js/utils.js, data-loader.js | OK |
| Modo Test 10K | src/js/gamestate.js | OK |
| CSVs reorganizados (9 archivos) | src/data/ | Refactor 2026-06-25 OK |
...
| Backup | Hacer backup ZIP |
| Verde = Bajo riesgo / Amarillo = Riesgo medio / Rojo = Alto riesgo |
| Test = Genera CSV de tests al completar |

---

## ORDEN DE EJECUCION RECOMENDADO

M1 -> M2 -> M3 -> M4 -> M5 -> BACKUP ->
M6 -> M7 -> M8 -> M9 -> BACKUP ->
A (Tablon) -> C (Misiones) -> BACKUP ->
M11 -> M12 -> E -> G -> BACKUP ->
M13a -> M13b -> I -> K -> L -> N -> BACKUP ->
M14 -> M15 -> M16 -> M17 -> M18 -> BACKUP ->
Q -> T -> V
(NX backlog: despues de todo lo anterior)

DEPENDENCIAS CLAVE:
  M7 (Prioridades colonos) = RUPTURA del sistema actual -- backup OBLIGATORIO antes
  M17 (Grano+paja)         = MIGRACION CRITICA state.wheat -> state.grain
  M13a (Calor)             = depende de Paso G (forja)
  M13b (Calor casas)       = depende de M12 + M13a
  M14 (Murallas)           = depende de Paso N (bandidos)
  M15 (Slots/tier)         = depende de M7
  M18 (Adobe)              = depende de M16 + M17 + M13b
  M12 (Estaciones)         = depende de M11

---

## PLAN PENDIENTE -- NUEVAS MECANICAS (M1-M18 + NX)

> Estas mecanicas se ejecutan ANTES de los pasos A-V segun el orden de arriba.
> Los pasos A-V originales (con su detalle completo) continuan mas abajo.
...
### PASO B — 💾 Backup ⬜
```
Pulsa el botón "💾 Backup ZIP" en el juego.
Mueve el ZIP a bkp/ y renómbralo: aetheria_bkp_vA_orders.zip
```
...
### PASO D — 💾 Backup ⬜
```
Pulsa "💾 Backup ZIP" → mueve a bkp/ → renombra: aetheria_bkp_vC_missions.zip
```
...
### PASO F — 💾 Backup ⬜
```
Pulsa "💾 Backup ZIP" → mueve a bkp/ → renombra: aetheria_bkp_vE_tools.zip
```
...
### PASO H — 💾 Backup ⬜
```
Pulsa "💾 Backup ZIP" → mueve a bkp/ → renombra: aetheria_bkp_vG_forge.zip
```
...
### PASO J — 💾 Backup ⬜
```
Pulsa "💾 Backup ZIP" → mueve a bkp/ → renombra: aetheria_bkp_vI_livestock.zip
```
...
### PASO M — 💾 Backup ⬜
```
Pulsa "💾 Backup ZIP" → mueve a bkp/ → renombra: aetheria_bkp_vL_armory.zip
```
...
### PASO P — 💾 Backup ⬜
```
Pulsa "💾 Backup ZIP" → mueve a bkp/ → renombra: aetheria_bkp_vO_raids.zip
```
...
### PASO S — 💾 Backup ⬜
```
Pulsa "💾 Backup ZIP" → mueve a bkp/ → renombra: aetheria_bkp_vR_devtools.zip
```
...
### PASO U — 💾 Backup final ⬜
```
Pulsa "💾 Backup ZIP" → mueve a bkp/ → renombra: aetheria_bkp_vT_balance.zip
```
