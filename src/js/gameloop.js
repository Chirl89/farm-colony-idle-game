...
function feedColonistsMorning() {
  ...
  updateStarvingColonists(state.fedNoneToday);
...
function feedColonistsEvening() {
  ...
  updateStarvingColonists(Math.ceil(remainingValue / mealNeed));
  ...
  if (state.lumberMills && CONFIG.ProductionRate && CONFIG.ProductionRate.lumbermill_prod) {
    ...
    state.lumberMills.forEach((m, idx) => {
      if (m.workerAssigned > 0 && !m.isUnderConstruction && !m.isUpgrading) {
        ...
        woodRate += (baseYield / duration) * multiplier * mult;
      }
    });
  }
  ...
  if (state.quarries && CONFIG.ProductionRate && CONFIG.ProductionRate.quarry_prod) {
    ...
    state.quarries.forEach((q, idx) => {
      if (q.workerAssigned > 0 && !q.isUnderConstruction && !q.isUpgrading) {
        ...
        const tier = q.tier || 1;
        const multiplier = tier === 1 ? 1.0 : (tier === 2 ? 1.1 : 1.2);
        stoneRate += (baseYield / duration) * multiplier * mult;
      }
    });
  }
    ...
    state.farms.forEach((f, idx) => {
      if (f.stage !== 'idle' && !f.isUnderConstruction && f.workerAssigned > 0) {
        ...
        if (crop) {
          ...
          // Multiplicador de agricultura del trabajador asignado
...
function gameTick() {
  ...
  state.candidateRotationElapsed += delta;
  ...
  const dayDelta = delta;
      ...
      if (state.townHall && (state.townHall.isUnderConstruction || state.townHall.isUpgrading) && !state.townHall.isPaused) {
        ...
        let workerSpeed = 0.0;
        if (state.townHall.builders && state.townHall.builders.length > 0) {
        ...
        if (Array.isArray(state[bt.key])) {
          state[bt.key].forEach((building, idx) => {
            if ((building.isUnderConstruction || building.isUpgrading) && !building.isPaused) {
              ...
              let workerSpeed = 0.0;
              if (building.builders && building.builders.length > 0) {
      ...
      if (woodcutters.length > 0 && CONFIG.BasicGathering && CONFIG.BasicGathering.wood_auto) {
        ...
        const woodAdded = ((cfg.yield * mult) / cfg.duration) * delta;
      ...
      if (miners.length > 0 && CONFIG.BasicGathering && CONFIG.BasicGathering.stone_auto) {
        ...
        const stoneAdded = ((cfg.yield * mult) / cfg.duration) * delta;
      ...
      if (foragers.length > 0 && CONFIG.BasicGathering && CONFIG.BasicGathering.berries_auto) {
        ...
        const foodAdded = ((cfg.yield * mult) / cfg.duration) * delta;
      ...
      if (Array.isArray(state.lumberMills) && CONFIG.ProductionRate && CONFIG.ProductionRate.lumbermill_prod) {
        state.lumberMills.forEach((b, idx) => {
          if (b.workerAssigned > 0 && !b.isUnderConstruction && !b.isUpgrading) {
            ...
            const tier = b.tier || 1;
            const multiplier = tier === 1 ? 1.0 : (tier === 2 ? 1.1 : 1.2);
            const woodPassive = ((millCfg.yield * multiplier * mult) / millDuration) * delta;
      ...
      if (Array.isArray(state.quarries) && CONFIG.ProductionRate && CONFIG.ProductionRate.quarry_prod) {
        state.quarries.forEach((b, idx) => {
          if (b.workerAssigned > 0 && !b.isUnderConstruction && !b.isUpgrading) {
            ...
            const tier = b.tier || 1;
            const multiplier = tier === 1 ? 1.0 : (tier === 2 ? 1.1 : 1.2);
            const stonePassive = ((quarryCfg.yield * multiplier * mult) / quarryDuration) * delta;
    ...
    if (Array.isArray(state.bonfires)) {
      state.bonfires.forEach((bonfire, idx) => {
        ...
        if (!bonfire.isUnderConstruction && bonfire.isRunning) {
          ...
          bonfire.elapsed += delta * speedMultiplier;
    ...
    if (Array.isArray(state.granaries)) {
      state.granaries.forEach((granary, idx) => {
        ...
        if (granary.isRunning) {
          ...
          granary.elapsed += delta * speedMultiplier;
          ...
          if (granary.elapsed >= targetDuration) {
            ...
            const produced = runningRecipe ? runningRecipe.yield : 1;
    ...
    state.farms.forEach((farm, idx) => {
      ...
      if (farm.needsWatering) {
        if (farm.workerAssigned > 0) {
          ...
          const farmingLvl = worker ? (worker.attributes.farming || 3) : 3;
          const farmingMult = getAttributeMult(farmingLvl) * (worker ? getColonistEfficiency(worker) : 1.0);
          farm.waterElapsed = (farm.waterElapsed || 0) + delta * farmingMult;
      ...
      if (farm.workerAssigned > 0) {
        if (farm.stage === 'grow' || farm.stage === 'grow2') {
          ...
          const farmingLvl = worker ? (worker.attributes.farming || 3) : 3;
          const farmingMult = getAttributeMult(farmingLvl) * (worker ? getColonistEfficiency(worker) : 1.0);
          elapsedIncrement = delta * farmingMult;
        }
      }
      ...
      if (farm.stageElapsed >= stageDuration) {
        ...
        if (farm.stage === 'plow') {
          ...
          const harvestedYield = crop.yield * tierMultiplier * farmingMult;
    ...
    if (state.colonists) {
      ...
      state.colonists.forEach(colonist => {
        ...
        if (attr) {
          ...
          colonist.attributeXP[attr] += (xpYield * delta) / dayDuration;
